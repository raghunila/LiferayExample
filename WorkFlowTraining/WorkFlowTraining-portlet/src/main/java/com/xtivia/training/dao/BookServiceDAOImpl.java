...
public class BookServiceDAOImpl implements BookServiceDAO {
    ...
    public void addBook(Book book, ServiceContext context) throws PortalException {
        try {
            
            BookLocalServiceUtil.addBook(book,context);
    ...
    public boolean isUniqueISBN(Long isbn) {
        ...
        try {
            if (BookLocalServiceUtil.findByIsbn(isbn) != null) {
                isUnique = false;
            }
        } catch (SystemException ex) {
            Logger.getLogger(BookServiceDAOImpl.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public Book getBook(Long isbn) {
        try {
            return  BookLocalServiceUtil.findByPrimaryKey(isbn);
